...
namespace Coalition.App_Data
{
    ...
    public class CRoom
    {
        ...
        public double[] GetGameSettings()
        {
            ...
            answer[2] = configuration.numOfRounds - currentRound;
            return answer;
        }
        ...
        private static void ResponseAsAi(Type configurationType, int timeInTheCurrentState, out RoundInfo ri, KeyValuePair<string, CRoom> room, out int aiIndex)
        {
            if (configurationType == Type.NE)
            {
                ri = room.Value.roundsInfo[room.Value.currentRound];
                aiIndex = 0;
                foreach (var item in ri.playersRoundInfo)
                {
                    ...
                    {
                        double acceptance = room.Value.configuration.AiDevision[room.Value.currentRound][aiIndex];
            ...
            else if(configurationType == Type.NORMAL)
            {
                ri = room.Value.roundsInfo[room.Value.currentRound];
                aiIndex = 0;
                foreach (var item in ri.playersRoundInfo)
        ...
        private static void PlayAi(Type configurationType, int timeInTheCurrentState, out RoundInfo ri, KeyValuePair<string, CRoom> room, out int aiIndex)
        {
            if (configurationType == Type.NE)
            {
                ri = room.Value.roundsInfo[room.Value.currentRound];
                aiIndex = 0;
                foreach (var shouldBeAIPlayer in ri.playersRoundInfo)
                {
                    if (shouldBeAIPlayer.Value.isAI && shouldBeAIPlayer.Value.isProposer && timeInTheCurrentState >= 10)
                    {
                        double[] offers = room.Value.configuration.AiDevision[room.Value.currentRound];
                        double[] weights = new double[ri.playersRoundInfo.Count];
                        int totalWeight = 10;
                        int i = 0;
                        foreach(var player in ri.playersRoundInfo)
                        {
                            weights[i++] = player.Value.weight;
                        }
                        int[] playerIndex = getPlayersToOffer(aiIndex, offers, weights, totalWeight);
                        double sumOfOfferExcludeAi = 0;
                        for(int j = 0; j < playerIndex.Length; j++)
                        {
                            if (j != aiIndex && playerIndex[j] == 1)
                                sumOfOfferExcludeAi += offers[j];
                        }
                        int index = 0;
                        ...
                        foreach (var player in ri.playersRoundInfo)
                        {
                            if(index == aiIndex)
                            {
                                ...
                                player.Value.playerResponse = PlayerResponse.None;
                            }
                            else if (playerIndex[index] == 1)
                            {
                                player.Value.playerOffer = offers[index];
                                player.Value.playerResponse = PlayerResponse.None;
                            }
                            index++;
                        }
        ...
        private static int[] getPlayersToOffer(int aiIndex, double[] offers, double[] weights, int totalWeight)
        {
            int numOfPlayers = weights.Length;
            int[] result = new int[numOfPlayers];
            result[aiIndex] = 1;
            double minOffer = 101;            
            double numOfIterations = Math.Pow(2, numOfPlayers - 1);
            for (int i = 0; i < numOfIterations; i++)
            {
                int[] temp = getArrayFromBinary(numOfPlayers, i, aiIndex);
                double sum = 0;
                for (int j = 0; j < numOfPlayers; j++)
                    if (temp[j] == 1 && j != aiIndex)
                        sum += offers[j];
                double weightCollected = 0;
                for (int j = 0; j < numOfPlayers; j++)
                    if (temp[j] == 1)
                        weightCollected += weights[j];
                if (sum < minOffer && weightCollected >= totalWeight)
                {
                    minOffer = sum;
                    result = temp;
                }
        ...
        private static int[] getArrayFromBinary(int numOfPlayers, int i, int aiIndex)
        {
            string source = Convert.ToString(i, 2).PadLeft(numOfPlayers, '0');
        ...
        private bool NextRound()
        {
            ...
            {
                if (currentRound >= configuration.numOfRounds - 1)
                { 
                    foreach (var item in roundsInfo[currentRound].playersRoundInfo)
                    {
                        Player.SetFutureStatus(item.Key, Player.Status.WaitingRoom);
